...
export default function FeatureProudcts() {
  ...
  let { createWishlist, setNumOfWishlistItems, deleteWishlistItem, getWishlist } = useContext(CartContext)
    ...
    <>
      {loading ? <div className={`${styles.spinner} m-auto mt-5 mb-5 `}></div> : <>
        <div className="container my-5 py-5">
          <div className="row">
            {wishlistLoaded && allProudcts.map((product) => (<div key={product.id} className="col-md-6 col-sm-10 col-s-8 col-lg-4 position-relative ">
              <div className="product p-2">
                <Link to={'/product-details/' + product.id}><img src={product.imageCover} className='w-100' alt="" />
                  <p className='text-main'>{product.category.name}</p>
                  <h3 className='h6'>{product.title.split(" ").splice(0, 2).join(' ')}</h3></Link>

                <div className="d-flex justify-content-between">
                  <p>{product.price} <span className='fs-6'>EGP</span></p>
                  <div className="">
                ...
                <button onClick={() => generateCart(product.id)} className={`${styles.button} w-100 bg-main border-sucsess`} >
                ...
                {wishlist.find((item) => item.id === product.id)  ? <><div className={`${styles.btnf} d-block position-absolute top-0 d-flex align-items-left`}>

                  <button className='' onClick={() => removeFromWishlist(product.id)} ><i class="fa-solid fa-heart fs-2"></i></button>
                </div></> : <><div className={`${styles.btn_nf}  position-absolute top-0 d-flex align-items-left`}>

                  <button onClick={() => generateWishlist(product.id)} ><i class="fa-solid fa-heart fs-2"></i></button>
                </div>  </>}
